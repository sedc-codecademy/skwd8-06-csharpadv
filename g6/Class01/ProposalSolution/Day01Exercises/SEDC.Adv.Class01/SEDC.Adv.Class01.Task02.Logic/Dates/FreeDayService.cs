...
namespace SEDC.Adv.Class01.Task02.Logic.Dates
{
    public class FreeDayService
    {
        private readonly List<int> _hollidaysNonLeapYear;
        public FreeDayService()
        {
            _hollidaysNonLeapYear = new List<int>
            {
    ...
    //1 January, 7 January, 20 April, 1 May, 25 May, 3 August, 8 September, 12 October, 23 October and 8 December are not working days as well
        ...
        public bool CheckIfDateIsNonWorkingDay2(DateTime date)
        {
            ...
            {
                return _hollidaysNonLeapYear.Contains(date.DayOfYear);
            }

            if(date.DayOfYear > (DateTime.DaysInMonth(date.Year, 1) + DateTime.DaysInMonth(date.Year, 2)))
            {
                return _hollidaysNonLeapYear.Contains(date.DayOfYear - 1);
            }
